...
- 展示 `reply_text/reply`、`emotion`、`tone`、`avatar_state`、TTS 和 memory 状态。
...
4. 打开 Settings，切换 Localhost，确认当前 URL 是 `http://127.0.0.1:3000` 并 Check。
5. 发送消息，验证真实 `/api/dialogue` 或自动 Mock contract fallback。
