...
后续 Codex / agent 接手项目时，必须先读根目录 `AGENTS.md`，再按 `docs/DOCS_INDEX.md` 的顺序读取长期记忆文档。

核心入口：

- `AGENTS.md`：强约束、禁止事项、测试和 Git 边界。
- `docs/DOCS_INDEX.md`：文档索引和阅读顺序。
- `docs/PROJECT_CONTEXT.md`：产品定位、阶段路线、当前做和不做。
- `docs/ARCHITECTURE.md`：SwiftUI、ViewModel、API client、renderer、Voice Output 边界。
- `docs/API_CONTRACT.md`：当前 iOS 消费的后端契约。
- `docs/CODEX_HANDOFF.md`：当前真实状态、风险、缺失和下一步。
...
6. 打开 Settings 查看 Mock/Localhost/LAN、Voice Output 和 Rive asset 状态。
